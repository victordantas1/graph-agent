...
2. **Comandos do registry são um comando só, sem operador de shell.** O que impede uma linha de YAML de virar execução arbitrária é **uma coisa só: `run_command` usar `shlex.split` e nunca `shell=True`**, no mesmo processo que roda com `bypassPermissions`. Sem shell, `&&`, `;` e `$(...)` não são operadores — são argumentos literais, e o comando simplesmente falha. A denylist do carregador (`&&`, `||`, `|`, `;`, `>`, `<`, `` ` ``, `$(`) **não é essa garantia**: é um erro cedo, no boot, para o operador que escreveu `cd app && npm ci` descobrir na hora em vez de num `npm ERR!` incompreensível. E é parcial — só cobre `CAMPOS_DE_COMANDO`, não `base`, `root` nem `copy_untracked`. *Corolário para quem mexer nisso depois:* nada aqui autoriza `shell=True` "porque a denylist filtra"; ela não filtra, e retirá-la não abre buraco nenhum, enquanto trocar o `shlex.split` abre todos. Onde um repo precisaria de `cd app && npm ci`, quem resolve é o campo `root`.

   > **Nota (revisão final da E3):** `base` passou a ter validador próprio no boot (`^[A-Za-z0-9][A-Za-z0-9._/-]*$`), e `prepare_workspace` valida `branch` com o mesmo padrão. Não é shell: é o parser de argv do próprio git, que leria `-b --help origin/develop` como flag. É defesa em profundidade, não substituto do `shlex.split`.
